...
def update_status(p: Union[models.Plan, models.PlanInstance], status: str, db: Session):
    p.status = status
    db.add(p)
    db.commit()
    db.refresh(p)
...
async def run_plan(ctx: dict, conn_id: int, plan_id: int):
    ...
    for db in ctx["db"]:
        ...
        for rule in plan_instance.plan.rules: # type: Rule
            ...
            for discovery in search(plan_instance.plan.connection, plan_instance.schema_list, rule):

...
export default {
  props: ['connectionId', 'schema_name', 'table_name', 'column_name'],
  components: {
  },
  mixins: [loaderMixin],
  data () {
  ...
  methods: {
    ...
    ...mapActions('mask', ['getMaskingFunctions', 'addMask', 'getMaskedColumns']),
  ...
  mounted () {
    ...
    this.startLoader()
    Promise.all([
      this.getMaskedColumns({ schema_name, table_name, column_name }),
      this.getSchemas(),
      this.getMaskingFunctions()
    ]).then(([columns, schemas, functions]) => {
      this.schemas = schemas
      this.functions = functions
      if (columns.length > 0) {
        const { schema_name, table_name, column_name, masking_function } = columns[0]
        let func = ''
        if (masking_function) {
          func = masking_function.split('(')[0]
        }
        this.payload = { ...this.payload, schema_name, table_name, column_name, func }
        this.getTables(schema_name).then(result => {
          this.tables = result
          this.getColumns({ schema_name, table_name }).then(result => {
            this.columns = result
